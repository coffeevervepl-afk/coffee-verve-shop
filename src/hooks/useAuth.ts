...
export function useAuth(): AuthState & {
  ...
  const refresh = useCallback(async () => {
    ...
    const [userRes, configRes] = await Promise.all([
      sb.from('shop_users')
        .select('id,email,name,phone,language,telegram,loyalty_level,spent_12m,referral_code,taste_profile,birthday,is_b2b,b2b_discount,min_discount_until,last_purchase_at')

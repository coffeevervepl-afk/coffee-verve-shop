    ...
    <div>
      {FILTER_GROUPS.map((g, gi) => (
        <div key={g.id} className={gi > 0 ? 'mt-5 border-t border-[#E8E7E3] pt-5' : ''}>
          <h4 className="mb-3 text-[14px] font-semibold uppercase tracking-wide text-[#3A2115]">
          ...
          <div className="flex flex-col gap-2.5">
            {g.options.map(o => {
              ...
              return (
                <label key={o.id}
                       className={`flex cursor-pointer items-center gap-2.5 text-[14px] ${dim ? 'text-[#B0AAA2]' : 'text-[#4A4540]'}`}>
                  <input type="checkbox" checked={checked} onChange={() => onToggle(o.id)}
                         className="h-4 w-4 shrink-0 accent-[#3A2115]" />
                  <span>
                    {t(`filters.${o.labelKey}`)}{' '}
                    <span className={dim ? 'text-[#B0AAA2]' : 'text-[#8A7A66]'}>({n})</span>
                  </span>
                </label>
              )
            })}

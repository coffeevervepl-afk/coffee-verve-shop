...
export default function FeaturedCarousel({ products, locale }: Props) {
  ...
  return (
    <section className="container pb-10">
      ...
      {/* Wrapped in its own margin (not the section's own pb-10, which the
          global .container class silently zeroes out via its `padding: 0 1rem`
          shorthand winning the cascade) so the gap to the next section actually renders. */}
      <div className="mb-10">
        <div className="group/carousel relative">
          ...
          >
            {products.map(p => (
              <div key={p.id} className="w-[85%] max-w-[340px] shrink-0 snap-start md:w-[calc((100%-48px)/4)] md:max-w-none">

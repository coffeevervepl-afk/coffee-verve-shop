...
export async function GET(req: NextRequest) {
  ...
  const [userRes, configRes, ordersRes] = await Promise.all([
    sb.from('shop_users')
      .select('id,email,name,loyalty_level,spent_12m,referral_code,taste_profile,birthday,is_b2b,b2b_discount,level_updated_at')
      .eq('email', email)
      .single(),
...
export async function PATCH(req: NextRequest) {
  const { email, taste_profile, birthday, name } = await req.json()
  if (!email) return NextResponse.json({ error: 'no_email' }, { status: 400 })
